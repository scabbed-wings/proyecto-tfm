...
def pos_ent(ent_atr):
    ...
    for ind, elem in enumerate(ent_atr):
        x, y = OBJ_SPACE, 2 * OBJ_SPACE + 2 * H_REL
...
def pos_rel_relx(num_id1, pos): #Posición para relaciones reflexivas
    ...
    if num_id1 in [0, 2]:
        ...
        conn_1[3] = y_rel if num_id1 == 2 else y_rel + 2 * H_REL
        conn_2[0], conn_2[2], conn_2[1] = x_rel + 2 * W_REL, pos[num_id1][0] + 2 * W_ENT, y_rel + H_ENT
        conn_2[3] = conn_1[1] + H_ENT
        pos_card_1[0], pos_card_2[0] = x_rel + W_REL - W_CARD, x_rel + W_REL + CARD_WS
        ...
        conn_1[3] = y_rel if num_id1 == 3 else y_rel + 2 * H_REL
        conn_2[0], conn_2[2], conn_2[1] = x_rel - 2 * W_REL, pos[num_id1][0], y_rel + W_REL
...
def pos_rel_asoc(num_id1, num_id2, pos): # Posición para relaciones asociativas
    ...
    if (num_id1 == 0 and num_id2 == 1) or (num_id1 == 2 and num_id2 == 3): # Relaciones horizontales id mas bajo a la izquierda
        ...
        pos_card_1[0], pos_card_2[0] = pos[num_id1][0] + 2 * W_ENT + CARD_WS, pos[num_id2][0] - W_CARD
        pos_card_1[1] = pos[num_id1][1] + H_ENT - CARD_HS if num_id2 == 1 else pos[num_id1][1] + H_ENT + CARD_HS
        pos_card_2[1] = pos[num_id2][1] + H_ENT - CARD_HS if num_id2 == 1 else pos[num_id2][1] + H_ENT + CARD_HS
        ...
        pos_card_1[1], pos_card_2[1] = pos[num_id1][1] + (H_ENT * 2) + CARD_HS, y_rel - CARD_HS
        pos_card_1[0] = pos[num_id1][0] + W_ENT - W_CARD if num_id1 == 0 else pos[num_id1][0] + W_ENT + CARD_WS 
        pos_card_2[0] = pos[num_id2][0] + W_ENT - W_CARD if num_id2 == 2 else pos[num_id2][0] + W_ENT + CARD_WS 
        ...
        if num_id1 in [0, 2]:
            ...
            conn_1[0], conn_1[2] = pos[4][0] + W_ENT, x_rel + W_REL
            conn_1[1] = pos[4][1] if num_id1 == 0 else pos[4][1] + 2 * H_ENT
            conn_1[3] = y_rel + 2 * H_REL if num_id1 == 0 else y_rel
            conn_2[0], conn_2[2] = pos[num_id1][0] + W_ENT, x_rel + W_REL
            conn_2[1] = pos[num_id1][1] + 2 * H_ENT if num_id1 == 0 else pos[num_id1][1]
            conn_2[3] = y_rel if num_id1 == 0 else y_rel + 2 * H_REL
            pos_card_1[0] = pos_card_2[0] = x_rel + W_REL - (W_CARD /2)
            pos_card_1[1], pos_card_2[1] = y_rel - CARD_HS, y_rel + (2 * H_REL + CARD_HS)
        elif num_id1 in [1, 3]:
            ...
            if num_id1 == 1:
                ...
                y_rel = ((pos[num_id1][1] + (pos[num_id2][1] + 2 * H_REL)) / 2.) - H_REL
            conn_1[0], conn_1[2] = pos[4][0] + 2 * W_ENT, x_rel + H_REL
            conn_1[1] = pos[4][1] if num_id1 == 1 else pos[4][1] + 2 * H_ENT
            conn_1[3] = y_rel + 2 * H_REL if num_id1 == 1 else y_rel
            conn_2[0], conn_2[2] = pos[num_id1][0] + W_ENT, x_rel + W_REL
            conn_2[1] = pos[num_id1][1] + 2 * H_ENT if num_id1 == 1 else pos[num_id1][1]
            conn_2[3] = y_rel if num_id1 == 1 else y_rel + 2 * H_REL
            pos_card_1[0] = pos_card_2[0] = x_rel + W_REL - (W_CARD /2)
            pos_card_1[1], pos_card_2[1] = y_rel - CARD_HS, y_rel + (2 * H_REL + CARD_HS)
