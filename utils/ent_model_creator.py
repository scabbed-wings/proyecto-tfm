...
def write_ent_atr(ent_atr, pos):
    ...
    for ind, elem in enumerate(ent_atr):
        num_comp += 1
        ent_s = f'''\n    <draw:custom-shape draw:name="Process {num_proc}" draw:style-name="gr5" draw:text-style-name="P1" xml:id="{elem[0]}" draw:id="{elem[0]}" draw:layer="layout" svg:width="{pos[ind][3] * 2}cm" svg:height="{pos[ind][4] * 2}cm" svg:x="{pos[ind][0]}cm" svg:y="{pos[ind][1]}cm">
        <text:p text:style-name="P1"><text:span text:style-name="T1">{elem[2]}</text:span></text:p>
        ...
        if len(elem[1]) > 0:
            for ind_atr, elem2 in enumerate(pos[ind][2]):
                num_id += 1
                num_comp += 1
                atr_s =  f'''\n    <draw:custom-shape draw:style-name="gr5" draw:text-style-name="P1" xml:id="id{num_id}" draw:id="id{num_id}" draw:layer="layout" svg:width="{elem2[2] * 2}cm" svg:height="{elem2[3] * 2}cm" svg:x="{elem2[0]}cm" svg:y="{elem2[1]}cm">
